...
public class FragBullet extends Bullet
{
    private int fragAmount;
    private float fragWeight;
    private float fragSpeed;

    public FragBullet(final Main main, final float x, final float y, final int width, final int height, final float angle, final float speed, final float alpha, final double damage, final double duration)
    {
    ...
    public void onCollision()
    {
        ...
        for (int i = 0; i < this.fragAmount; ++i)
        {
            ...
            GameState.a.add(new SmallBullet(this.main, this.x, this.y, fragWidth, fragHeight, angle, speed, 100.0f, fragDamage, fragDuration));
        }
    }
