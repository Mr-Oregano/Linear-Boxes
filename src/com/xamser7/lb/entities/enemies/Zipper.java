...
{
    public Zipper(final Main main, final float x, final float y, final int width, final int height, final float angle, final float speed, final float alpha)
    {
        super(main, x, y, width, height, angle, speed, alpha);
        this.health = (int) Math.round(Math.pow(Game.Level / 5.0, 1.31) + 5.0);
        this.prize = (int) Math.round(Math.pow(Game.Level / 5.0, 2.0) + 1000.0);
    ...
    public void render(final Graphics2D g)
    {
        ...
        g.drawImage(Assets.zipper, (int) this.x, (int) this.y, this.width, this.height, null);
        this.showHealth(g);
    }
}
